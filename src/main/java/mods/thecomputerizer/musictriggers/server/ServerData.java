...
public class ServerData {
    ...
    private boolean calculateMob(Table mobTrigger, ServerPlayerEntity player, BlockPos pos) {
        ...
        if (resources.contains("BOSS")) {
            List<ServerBossInfo> correctBosses = this.bossInfo.stream().filter(
                    info -> resources.size()==1 || partiallyMatches(info.getName().getString(),resources)).collect(Collectors.toList());
    ...
    private boolean checkMobs(ServerPlayerEntity player, BlockPos pos, int num, int range, List<String> whiteList,
    ...
    private boolean checkEntityName(LivingEntity entity, List<String> whiteList, List<String> blackList) {
        ...
        if(blackList.contains("MOB") &&!(entity instanceof MobEntity)) return false;
