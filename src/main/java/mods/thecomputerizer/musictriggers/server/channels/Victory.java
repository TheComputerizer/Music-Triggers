...
class Victory {
    ...
    private final Map<Table,HashMap<EntityLivingBase,MutableInt>> ENTITY_CACHE;
    private final Map<Table,HashMap<EntityPlayerMP,MutableInt>> PLAYER_CACHE;
    ...
    protected boolean runCalculation(ServerTriggerStatus calculator) {
        for(Table reference : this.REFERNCES) {
            ...
            if(this.ENTITY_CACHE.containsKey(reference)) {
                ...
                if (num > 0) {
                    boolean pass = false;
                    HashMap<EntityLivingBase,MutableInt> entities = this.ENTITY_CACHE.get(reference);
                    if (entities.size()==0) continue;
                    if (entities.size() >= num) {
                        float percent = calculator.getParameterFloat(reference,"victory_percentage") / 100f;
                        float count = 0;
                        for (Map.Entry<EntityLivingBase,MutableInt> entityEntry : entities.entrySet())
                            if (entityEntry.getKey().isDead || entityEntry.getKey().getHealth() <= 0f)
                                count++;
                ...
                float num = (float)(calculator.getParameterInt(reference,"level"));
                if (num > 0) {
                    HashMap<EntityPlayerMP,MutableInt> players = this.PLAYER_CACHE.get(reference);
                    if (players.size() == 0) continue;
                    float percent = calculator.getParameterFloat(reference,"victory_percentage") / 100f;
                    float count = 0;
                    for (Map.Entry<EntityPlayerMP, MutableInt> playerEntry : players.entrySet())
                        if (playerEntry.getKey().isDead || playerEntry.getKey().getHealth() <= 0f)
                            count++;
