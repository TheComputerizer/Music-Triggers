...
public class ServerChannelManager {
    ...
    public static void initialize(IResourceManager manager, InputStream resource) throws IOException {
        SERVER_CHANNELS.clear();
        Holder channels = TomlUtil.readFully(resource);
        ...
        for (Table channel : channels.getTables().values()) {
            if (verifyChannelName(channel.getName()))
                SERVER_CHANNELS.put(channel.getName(), new ServerChannel(channel,manager));
            else
