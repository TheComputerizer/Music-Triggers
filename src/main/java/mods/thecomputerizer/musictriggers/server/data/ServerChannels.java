...
public class ServerChannels {
    ...
    public ServerChannels(PacketBuffer buf) {
    ...
    private void runChecks() {
        ...
        this.bossInfo.removeIf(info -> info.getPercent()<=0 || !info.isVisible() || info.getName().getString().matches("Raid"));
    ...
    private boolean calculateMob(Table mobTrigger, ServerPlayerEntity player, BlockPos pos) {
        ...
        if (resources.contains("BOSS")) {
            HashSet<ServerBossInfo> matchedBosses = new HashSet<>(resources.size() == 1 ? this.bossInfo : this.bossInfo.stream().filter(
            ...
            if(Objects.nonNull(victory)) {
                if(pass)
                    for(ServerBossInfo info : matchedBosses)
    ...
    private boolean checkMobs(Table trigger, ServerPlayerEntity player, BlockPos pos, int num, int range, List<String> resources,
    ...
    private boolean checkSpecifics(HashSet<LivingEntity> entities, int num, ServerPlayerEntity player, boolean target,
    ...
    private boolean checkTarget(HashSet<LivingEntity> entities, int num, boolean target, float ratio, ServerPlayerEntity player) {
    ...
    private boolean checkBossHealth(HashSet<ServerBossInfo> bars, int num, float health, float ratio) {
        if(health>=100 || ratio<=0) return true;
        float counter = 0f;
        for(ServerBossInfo bar : bars)
