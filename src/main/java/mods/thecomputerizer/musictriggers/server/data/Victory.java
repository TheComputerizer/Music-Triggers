...
class Victory {
    ...
    private final Map<Table,HashSet<Tuple<ServerBossInfo,MutableInt>>> BOSS_CACHE;
    private final Map<Table,HashSet<Tuple<ServerPlayerEntity,MutableInt>>> PLAYER_CACHE;
    ...
    protected boolean runCalculation() {
        for(Table reference : this.REFERNCES) {
            ...
            if(this.ENTITY_CACHE.containsKey(reference)) {
                ...
                if (num > 0) {
                    boolean pass = false;
                    HashSet<Tuple<ServerBossInfo,MutableInt>> bossBars = this.BOSS_CACHE.get(reference);
                    if (bossBars.size()==0) continue;
                    if(bossBars.size() >= num) {
                        float percent = reference.getValOrDefault("victory_percentage", 100f) / 100f;
                        float count = 0;
                        for(Tuple<ServerBossInfo,MutableInt> bossTuple : bossBars)
                            if(bossTuple.getA().getPercent() <= 0f)
                                count++;
                        if(count / num >= percent) pass = true;
                    }
                ...
                int num = reference.getValOrDefault("level", 0);
                if (num > 0) {
                    HashSet<Tuple<ServerPlayerEntity, MutableInt>> players = this.PLAYER_CACHE.get(reference);
                    if (players.size() == 0) continue;
                    float percent = reference.getValOrDefault("victory_percentage", 100f) / 100f;
                    float count = 0;
                    for (Tuple<ServerPlayerEntity, MutableInt> playerTuple : players)
