...
public class MusicPlayer {
    ...
    public static void onTick() {
        if(!reloading && !mc.isPaused()) {
            ...
            if (!paused && mc.isWindowFocused()) {
                if (curMusic!=null && sh.soundSystem.sources.get(curMusic)!=null && sh.soundSystem.sources.get(curMusic).source!=null && !fadingIn && !fadingOut && !reverseFade && (linkedFadingOut.get(curLinkNum)==null || !linkedFadingOut.get(curLinkNum)) && (linkedFadingIn.get(curLinkNum)==null || !linkedFadingIn.get(curLinkNum)))
                    Objects.requireNonNull(sh.soundSystem.sources.get(curMusic).source).setVolume(mc.options.getSoundVolume(SoundCategory.MUSIC)*volumeLinker.get(curLinkNum));
                ...
                if (curTrack != null && sh.isPlaying(curMusic) && mc.currentScreen instanceof GuiCurPlaying) ((GuiCurPlaying) mc.currentScreen).setSlider(GuiCurPlaying.getSongPosInSeconds(curMusic));
                    ...
                    if (holder != null && !holder.isEmpty() && !playing) {
                        ...
                        if (curTrackList == null && !finish) {
                            ...
                            for (String command : ConfigCommands.commandMap.keySet()) {
                                if (new HashSet<>(curTrackList).containsAll(ConfigCommands.commandMap.get(command)) && new HashSet<>(ConfigCommands.commandMap.get(command)).containsAll(curTrackList))
                            ...
                            if (MusicPicker.shouldChange || !curTrackList.equals(holder)) {
                                ...
                                if (trimmedList.size() >= 1) {
                                    ...
                                    if (trimmedList.size() > 1 && curTrack != null) {
                                        ...
                                        if (j >= 1000)
                                            MusicTriggersCommon.logger.warn("Attempt to get non duplicate song passed 1000 tries! Forcing current song " + ConfigToml.songholder.get(curTrack) + " to play.");
