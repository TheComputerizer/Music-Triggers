        ...
        if(!reloading && !mc.isPaused()) {
                        ...
                        for (int z = MusicPicker.player.zChunk - 3; z <= MusicPicker.player.zChunk + 3; z++) {
                            ...
                            for (BlockPos b : currentChunkTEPos) {
                                if (MusicPicker.world.getChunk(x, z).getBlockEntity(b) instanceof JukeboxTileEntity) {
                                    ...
                                    if (te != null && te.getBlockState().getValue(JukeboxBlock.HAS_RECORD))
                                        playing = true;
                                }
                            }
                        }
            ...
            if (!reloading && tickCounter % 2 == 0 && !paused && mc.isWindowActive()) {
                if (curMusic!=null && sh.soundEngine.instanceToChannel.get(curMusic)!=null && sh.soundEngine.instanceToChannel.get(curMusic).channel!=null && !fadingIn && !fadingOut && !reverseFade && (linkedFadingOut.get(curLinkNum)==null || !linkedFadingOut.get(curLinkNum)) && (linkedFadingIn.get(curLinkNum)==null || !linkedFadingIn.get(curLinkNum)))
                    Objects.requireNonNull(sh.soundEngine.instanceToChannel.get(curMusic).channel).setVolume(mc.options.getSoundSourceVolume(SoundCategory.MUSIC)*volumeLinker.get(curLinkNum));
                ...
                if (curTrack != null && curMusic!=null && sh.isActive(curMusic) && ConfigToml.loopPoints.containsKey(curTrack)) {
                ...
                if (curTrack != null && curMusic!=null && sh.isActive(curMusic) && mc.screen instanceof GuiCurPlaying) ((GuiCurPlaying) mc.screen).setSlider(GuiCurPlaying.getSongPosInSeconds(curMusic));
                ...
                if (fadingOut && !reverseFade) {
                    ...
                    if(entry!=null && entry.channel!=null) {
                        if (tempFadeOut >= savedFadeOut) {
                            fadingOut = false;
                            reverseFade = false;
                            entry.channel.setVolume(saveVolOut * mc.options.getSoundSourceVolume(SoundCategory.MUSIC));
                            tempFadeOut = 0;
                        } else {
                            float calculatedVolume = saveVolOut * (float) (((double) tempFadeOut) / ((double) savedFadeOut));
                            calculatedVolume = calculatedVolume * mc.options.getSoundSourceVolume(SoundCategory.MUSIC);
