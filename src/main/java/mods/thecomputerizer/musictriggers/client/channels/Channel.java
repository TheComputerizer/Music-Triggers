...
public class Channel implements IChannel {
    public static final KeyBinding GUI = new KeyBinding("key.musictriggers.menu", KeyConflictContext.UNIVERSAL,
            InputMappings.Type.KEYSYM, GLFW.GLFW_KEY_R, "key.categories.musictriggers");
    ...
    private static ConfigRedirect makeRedirect(Table info, boolean isResourcePack, IResourceManager manager) throws IOException {
    ...
    private static ConfigJukebox makeJukebox(Table info, boolean isResourcePack, IResourceManager manager) throws IOException {
     ...
     * Client Config
     */
    public Channel(Table info, boolean isResourcePack, IResourceManager manager) throws IOException {
    ...
    public Channel(Table info, Holder main, Holder transitions, Holder commands, Holder toggles,
        ...
        this.category = SoundCategory.BY_NAME.getOrDefault(category, SoundCategory.MUSIC);
    ...
    private boolean checkAudio() {
        return Minecraft.getInstance().options.getSoundSourceVolume(SoundCategory.MASTER) > 0
                && Minecraft.getInstance().options.getSoundSourceVolume(this.category) > 0 && this.AUDIO_QUEUE.get() <=0;
    }
    ...
    private float getChannelVolume() {
        ...
        float volume = settings.getSoundSourceVolume(SoundCategory.MASTER);
        return getCategory()==SoundCategory.MASTER ? volume : volume*settings.getSoundSourceVolume(getCategory());
    }
        ...
        try {
            ...
            if (!this.loadedTracks.containsKey(id)) {
                ...
                for(ResourcePackInfo packInfo : Minecraft.getInstance().getResourcePackRepository().getSelectedPacks()) {
                    ...
                    if(pack.getNamespaces(ResourcePackType.CLIENT_RESOURCES).contains(namespace) &&
                            pack.hasResource(ResourcePackType.CLIENT_RESOURCES,source)) {
                        URL url = pack.getClass().getResource("/" + ResourcePackType.CLIENT_RESOURCES.getDirectory() +
                                "/" + source.getNamespace() + "/" + source.getPath());
                        if (url != null && (url.getProtocol().equals("jar") || FolderPack.validatePath(new File(url.getFile()),
                                "/" + ResourcePackType.CLIENT_RESOURCES.getDirectory() + "/" + source.getNamespace() + "/" + source.getPath())))
    ...
    public void encodeDynamic(PacketBuffer buf) {
        MusicPicker.Info info = this.picker.getInfo();
