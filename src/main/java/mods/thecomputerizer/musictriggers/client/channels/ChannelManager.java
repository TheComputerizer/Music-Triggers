...
public class ChannelManager {
    private static final Set<String> VALID_FILE_EXTENSIONS = new HashSet<>(Arrays.asList(".acc",".flac",".m3u",
    ...
    private static Holder getChannelsHolder(File channelsFile) throws IOException {
        ...
        try {
            Resource resource = Minecraft.getInstance().getResourceManager().getResourceOrThrow(channelsResource);
            isResourceControlled = true;
            return TomlUtil.readFully(resource.open());
    ...
    public static void tickChannels(long millis) {
        if(millis==50) {
            if(!isPlayerAssigned) {
