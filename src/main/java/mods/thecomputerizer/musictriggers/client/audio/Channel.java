...
public class Channel {
    ...
    public Channel(String channel, boolean pausedByJukeBox, boolean overrides) {
        ...
        this.category = SoundSource.valueOf(this.channel.toUpperCase(Locale.ROOT));
    ...
    public void tickSlow() {
        ...
        if (!this.picker.getInfo().getCurrentSongList().isEmpty()) {
            ...
            if (isPlaying() && (mc.options.getSoundSourceVolume(this.category) == 0 || mc.options.getSoundSourceVolume(SoundSource.MASTER) == 0)) {
                stopTrack();
                if (this.fadingOut) {
    ...
    public void renderCards(Minecraft mc) {
        ...
        for (int i : this.transitions.titlecards.keySet()) {
            ...
            if (pass && mc.player != null) {
                ...
                if(!this.transitions.titlecards.get(i).getTitles().isEmpty()) mc.gui.setTitle((new TextComponent(this.transitions.titlecards.get(i).getTitles().get(ThreadLocalRandom.current().nextInt(0, this.transitions.titlecards.get(i).getTitles().size())))).withStyle(ChatFormatting.getByName(this.transitions.titlecards.get(i).getTitlecolor())));
                if(!this.transitions.titlecards.get(i).getSubTitles().isEmpty()) mc.gui.setSubtitle((new TextComponent(this.transitions.titlecards.get(i).getSubTitles().get(ThreadLocalRandom.current().nextInt(0, this.transitions.titlecards.get(i).getSubTitles().size())))).withStyle(ChatFormatting.getByName(this.transitions.titlecards.get(i).getSubtitlecolor())));
    ...
    private float getChannelVolume() {
        ...
        if(SoundSource.valueOf(this.channel.toUpperCase(Locale.ROOT))==SoundSource.MASTER) return master;
        else return master*Minecraft.getInstance().options.getSoundSourceVolume(SoundSource.valueOf(this.channel.toUpperCase(Locale.ROOT)));
    }
        ...
        try {
            ...
            if (!this.loadedTracks.containsKey(id)) {
                ...
                for(Pack packInfo : Minecraft.getInstance().getResourcePackRepository().getSelectedPacks()) {
                    ...
                    if(pack.getNamespaces(PackType.CLIENT_RESOURCES).contains(namespace) && pack.hasResource(PackType.CLIENT_RESOURCES,source)) {
                        MusicTriggers.logger.info("The resource pack that has " + source + " is " + pack.getName() + " under class " + pack.getClass().getName());
                        URL url = pack.getClass().getResource("/" + PackType.CLIENT_RESOURCES.getDirectory() + "/" + source.getNamespace() + "/" + source.getPath());
                        if (url != null && (url.getProtocol().equals("jar") || FolderPackResources.validatePath(new File(url.getFile()), "/" + PackType.CLIENT_RESOURCES.getDirectory() + "/" + source.getNamespace() + "/" + source.getPath())))
                            sourcePath = url.getPath();
                        else {
                            if (pack instanceof AbstractPackResources resourcePack) {
                                String resource = String.format("%s/%s/%s", PackType.CLIENT_RESOURCES.getDirectory(), source.getNamespace(), source.getPath());
                                if (pack instanceof PathResourcePack modResource && !(namespace.matches("minecraft") || namespace.matches("realms"))) {
