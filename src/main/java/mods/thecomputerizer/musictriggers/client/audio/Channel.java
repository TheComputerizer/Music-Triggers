        ...
        for (int i : this.transitions.titlecards.keySet()) {
            ...
            if (pass && mc.player != null) {
                ...
                if(!this.transitions.titlecards.get(i).getTitles().isEmpty())
                    mc.gui.setTitles((new StringTextComponent(this.transitions.titlecards.get(i).getTitles()
                            .get(ThreadLocalRandom.current().nextInt(0, this.transitions.titlecards.get(i)
                                    .getTitles().size())))).withStyle(TextFormatting.getByName(
                                            this.transitions.titlecards.get(i).getTitlecolor())), null, 5, 20, 20);
                if(!this.transitions.titlecards.get(i).getSubTitles().isEmpty())
                    mc.gui.setTitles(null, (new StringTextComponent(this.transitions.titlecards.get(i)
                            .getSubTitles().get(ThreadLocalRandom.current().nextInt(0,
                                    this.transitions.titlecards.get(i).getSubTitles().size())))).withStyle(
                                            TextFormatting.getByName(this.transitions.titlecards.get(i).getSubtitlecolor())), 5, 20, 20);
            ...
            if (!this.loadedTracks.containsKey(id)) {
                ...
                for(ResourcePackInfo packInfo : Minecraft.getInstance().getResourcePackRepository().getSelectedPacks()) {
                    ...
                    if(pack.getNamespaces(ResourcePackType.CLIENT_RESOURCES).contains(namespace) && pack.hasResource(ResourcePackType.CLIENT_RESOURCES,source)) {
                        MusicTriggers.logExternally(Level.INFO, "The resource pack that has " + source + " is " + pack.getName() + " under class " + pack.getClass().getName());
