...
public class Trigger {
    ...
    private static final Map<String, BiFunction<Trigger, ClientPlayerEntity, Boolean>> TRIGGER_CONDITIONS = new HashMap<>();
    ...
    private static void loadDefaultTriggers() {
        ...
        addTrigger("pet",false,makeParameterSet(false,"detection_range","detection_y_ratio"),
            ...
            for (LivingEntity ent : player.level.getEntitiesOfClass(LivingEntity.class,
                    new AxisAlignedBB(player.getX()-range, player.getY()-((float)range*yRatio), player.getZ()-range,
        ...
        addTrigger("gui",false,makeParameterSet(true,"resource_name"),
                Arrays.asList("identifier","resource_name"),new ArrayList<>(),(trigger,player) -> {
                    ...
                    return (Objects.nonNull(curScreen) && (trigger.getResource().isEmpty() ||
                            ...
                            (trigger.getResource().contains("CREDITS") && curScreen instanceof WinGameScreen)));
                },true);
        ...
        addTrigger("gamestage",false,Collections.singletonList("gamestages"),
                ...
                Arrays.asList("identifier","resource_name"),new ArrayList<>(),
                (trigger,player) -> trigger.whitelistHelper(GameStageHelper.hasAnyOf(player,trigger.getResource())),true);
        ...
        addTrigger("moon",false, Collections.singletonList("enhancedcelestials"),
                ...
                (trigger,player) -> {
                    ...
                    if(Objects.nonNull(lunarContext) && lunarContext.getCurrentEvent() instanceof Moon)
                        return trigger.checkResourceMatch(lunarContext.getCurrentEvent().getKey(),false);
                    return false;
                },true);
        addTrigger("rainintensity",false,Collections.singletonList("dsurround"),
                ...
                (trigger,player) -> WorldUtils.getRainStrength(player.level, 1f)>
                        (((float)trigger.getParameterInt("level"))/100f),true);
        ...
        addTrigger("season",false,Collections.singletonList("sereneseasons"),
                ...
                (trigger,player) -> trigger.seasonHelper(SeasonHelper.getSeasonState(player.level).getSeason()),true);
    }
    ...
    public static void addTrigger(String name, boolean isServerSide, Set<String> acceptedParameters,
                                  BiFunction<Trigger, ClientPlayerEntity, Boolean> activationFunction,
    ...
    public static void encodeDefaultParameters(PacketBuffer buf) {
    ...
    public static void updateEffectCache(ClientPlayerEntity player) {
        if(Objects.isNull(player)) return;
        Set<String> activeEffects = player.getActiveEffects().stream()
                .map(instance -> Objects.isNull(instance) ? null : instance.getEffect().getRegistryName().toString())
    ...
    public boolean checkRiding(ClientPlayerEntity player) {
        ...
        else if (Objects.isNull(ForgeRegistries.ENTITIES.getKey(riding.getType()))) return false;
        return checkResourceMatch(ForgeRegistries.ENTITIES.getKey(riding.getType()).toString(),false);
    }


    public boolean checkStat(ClientPlayerEntity player) {
        if(this.cache.isTypeCached("statistic")) {
            Minecraft mc = Minecraft.getInstance();
            if (Objects.nonNull(player) && Objects.nonNull(mc.getConnection())) {
                mc.getConnection().send(new CClientStatusPacket(CClientStatusPacket.State.REQUEST_STATS));
    ...
    public boolean checkInventory(ClientPlayerEntity player) {
    ...
    private boolean checkNBT(ItemStack stack, String nbt) {
        ...
        try {
            ...
            if(NBT_MODES.contains(parts[0])) {
                ...
                CompoundNBT data = stack.serializeNBT();
                switch (parts[0]) {
                    case "KEY_PRESENT" : {
                    ...
                    default: {
                        ...
                        if(finalVal instanceof  ByteNBT) {
                            ...
                            pass = String.valueOf(((NumberNBT)finalVal).getAsDouble()).trim().toLowerCase()
    ...
    public boolean checkForTile(ClientPlayerEntity player) {
        ...
        for(TileEntity loadedTile : player.level.blockEntityList) {
            for(TileEntityType<?> tileType : this.cache.blockEntityCache) {
                if(loadedTile.getType()==tileType) {
                    BlockPos tilePos = loadedTile.getBlockPos();
    ...
    class Cache {
        ...
        private final Set<TileEntityType<?>> blockEntityCache;
        ...
        private void initCache() {
            ...
            if(isTrigger("blockentity")) {
                for(String resource : this.resourceCache) {
                    ...
                    if(ForgeRegistries.TILE_ENTITIES.containsKey(location))
                        this.blockEntityCache.add(ForgeRegistries.TILE_ENTITIES.getValue(location));
                }
        ...
        private boolean statHasValidName(Stat<?> stat) {
            ...
            if(value instanceof Block) {
                ResourceLocation res = ((Block)value).getRegistryName();
                return Objects.nonNull(res) && checkResourceMatch(res.toString(),false);
            }
            if(value instanceof Item) {
                ResourceLocation res = ((Item)value).getRegistryName();
                return Objects.nonNull(res) && checkResourceMatch(res.toString(),false);
            }
            if(value instanceof EntityType<?>) {
                ResourceLocation res = ((EntityType<?>)value).getRegistryName();
                return Objects.nonNull(res) && checkResourceMatch(res.toString(),false);
            }
            return false;
        }
    ...
    public static final class DefaultParameter {
        ...
        public DefaultParameter(PacketBuffer buf) {
