...
public class Trigger {
    ...
    private static final String[] allTriggers = new String[]{"menu","generic","difficulty","time","light","height",
            ...
            "advancement","statistic","command","gamestage","bloodmoon","harvestmoon","fallingstars", "rainintensity",
            "tornado","hurricane","sandstorm","season","raid","bluemoon","moon","acidrain","blizzard","cloudy",
            "lightrain"};
    private static final String[] acceptedTriggers = new String[]{"menu","generic","difficulty","time","light","height",
            "raining","storming","snowing","lowhp","dead", "creative","spectator","riding","pet","underwater","elytra",
            "fishing","drowning","home", "dimension","biome", "structure","mob","victory","gui","effect","zones","pvp",
            "advancement","statistic","command","gamestage","bloodmoon","harvestmoon","bluemoon","rainintensity",
            "acidrain","blizzard","cloudy","lightrain","season"};
    private static final String[] modtriggers = new String[]
            {"gamestage","bloodmoon","harvestmoon","bluemoon","rainintensity","acidrain","blizzard","cloudy","lightrain",
                    "season"};
    ...
    private static final HashMap<String, BiFunction<Trigger, ClientPlayerEntity, Boolean>> triggerConditions = setTriggerConditions();
    ...
    private static HashMap<String, BiFunction<Trigger, ClientPlayerEntity, Boolean>> setTriggerConditions() {
        HashMap<String, BiFunction<Trigger, ClientPlayerEntity, Boolean>> ret = new HashMap<>();
        ...
        ret.put("pet",(trigger,player) -> {
            ...
            for (LivingEntity ent : player.level.getEntitiesOfClass(LivingEntity.class,
                    new AxisAlignedBB(player.getX()-16, player.getY()-8, player.getZ()-16,
                            player.getX()+16, player.getY()+8, player.getZ()+16))) {
                if ((ent instanceof TameableEntity && ent.serializeNBT() != null &&
        ...
        ret.put("bloodmoon",(trigger,player) -> {
            if(ModList.get().isLoaded("enhancedcelestials")) {
                ...
                return lunarContext != null && lunarContext.getCurrentEvent() instanceof BloodMoon;
            }
            return false;
        });
        ret.put("harvestmoon",(trigger,player) -> {
            if(ModList.get().isLoaded("enhancedcelestials")) {
                LunarContext lunarContext = ((EnhancedCelestialsWorldData) player.level).getLunarContext();
                return lunarContext != null && lunarContext.getCurrentEvent() instanceof HarvestMoon;
            }
            return false;
        });
        ret.put("bluemoon",(trigger,player) -> {
            if(ModList.get().isLoaded("enhancedcelestials")) {
                LunarContext lunarContext = ((EnhancedCelestialsWorldData) player.level).getLunarContext();
                return lunarContext != null && lunarContext.getCurrentEvent() instanceof BlueMoon;
            }
            return false;
        });
        ret.put("moon",(trigger,player) -> {
            if(ModList.get().isLoaded("enhancedcelestials")) {
                LunarContext lunarContext = ((EnhancedCelestialsWorldData) player.level).getLunarContext();
                if(lunarContext != null && lunarContext.getCurrentEvent() instanceof Moon)
                    return lunarContext.getCurrentEvent().getKey().contains(trigger.getResource());
            }
            return false;
        });
        ret.put("rainintensity",(trigger,player) -> ModList.get().isLoaded("dsurround") &&
                WorldUtils.getRainStrength(player.level, 1F)>(((float)trigger.getParameterInt("level"))/100f));
        ret.put("acidrain",(trigger,player) -> {
        ...
        ret.put("blizzard",(trigger,player) -> {
            if(ModList.get().isLoaded("betterweather")) {
                BetterWeatherWorldData weatherdata = (BetterWeatherWorldData) player.level;
                return Objects.nonNull(weatherdata.getWeatherEventContext()) &&
        ...
        ret.put("lightrain",(trigger,player) -> {
            if(ModList.get().isLoaded("betterweather")) {
                BetterWeatherWorldData weatherdata = (BetterWeatherWorldData) player.level;
                return Objects.nonNull(weatherdata.getWeatherEventContext()) &&
