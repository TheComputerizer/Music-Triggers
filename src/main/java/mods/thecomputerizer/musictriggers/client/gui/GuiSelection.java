...
public class GuiSelection extends GuiSuperType {
    ...
    public static class MonoElement extends Element {
        ...
        public MonoElement(String id, int index, String display, List<String> hoverText, Consumer<GuiSelection> onClick,
            ...
            this.hoverText = hoverText.stream().map(line -> (Component) MutableComponent.create(new LiteralContents(line))).toList();
    ...
    public static class DualElement extends Element {
        ...
        public DualElement(String key, String val, int index, List<String> hoverTextKey, List<String> hoverTextVal,
            ...
            this.key = key;
            this.val = val;
            this.hoverTextKey = hoverTextKey.stream().map(line -> (Component)MutableComponent.create(new LiteralContents(line))).toList();
            this.hoverTextVal = hoverTextVal.stream().map(line -> (Component)MutableComponent.create(new LiteralContents(line))).toList();
            this.onDelete = onDelete;
            this.onSave = onSave;
