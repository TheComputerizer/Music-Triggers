...
public class GuiParameters extends GuiSuperType {
    ...
    public static class Parameter {
        ...
        private List<Component> varNameHover() {
            return Collections.singletonList(Component.literal(this.varName));
        }
            ...
            if(parent.isActive(parent)) {
                if (wasRightSide) {
                    if(Objects.isNull(this.var) && Objects.nonNull(this.specialClickFunc) && this.displayHover)
                        ...
                        if (this.selected)
                            this.descLines = GuiUtil.howManyLinesWillThisBe(Minecraft.getInstance().font,this.description,left,right);
                    }
                    return this.selected;
        ...
        public List<Component> drawVariableElement(PoseStack matrix, Vector3f topLeft, int spacing, int mouseX, int mouseY,
            ...
            if(this.var.getAsBool(true).isPresent()) {
                ...
                if(this.var.getName().matches("channel")) return Collections.singletonList(Component.literal(this.var.get().toString()));
                ...
                return ret.stream().map(Component::literal).collect(Collectors.toList());
            } else {
