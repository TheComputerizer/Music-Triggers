...
public class MusicPicker {
    ...
    public Packeted querySongList() {
        ...
        if(this.player == null) {
            if (this.mc.screen!=null && this.mc.level==null && this.handler.TriggerIdentifierMap.get("menu") != null) {
    ...
    public List<String> playableEvents(Packeted packet) {
        ...
        try {
            ...
            for (LivingEntity ent : world.getEntitiesOfClass(LivingEntity.class, new AABB(player.getX() - 16, player.getY() - 8, player.getZ() - 16, player.getX() + 16, player.getY() + 8, player.getZ() + 16))) {
                if ((ent instanceof TamableAnimal && ent.serializeNBT() != null && ent.serializeNBT().getString("Owner").matches(player.getStringUUID())) && this.handler.TriggerIdentifierMap.get("pet") != null) {
                    crashHelper = "pet";
                    events.add("pet");
            ...
            if (this.handler.TriggerIdentifierMap.get("gui") != null) {
                ...
                for (String identifier : this.handler.TriggerIdentifierMap.get("gui").keySet()) {
                    ...
                    } else if (this.handler.TriggerInfoMap.get("gui-" + identifier)[9].matches("CREDITS") && mc.screen instanceof WinScreen) {
                        if (!events.contains("gui-" + identifier)) {
                            events.add("gui-" + identifier);
                            dynamicSongs.put("gui-" + identifier, this.handler.TriggerIdentifierMap.get("gui").get(identifier));
                            dynamicPriorities.put("gui-" + identifier, MusicTriggers.randomInt(this.handler.TriggerInfoMap.get("gui-" + identifier)[0]));
