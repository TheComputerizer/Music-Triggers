...
public class MusicTriggersRecord extends Item {
    ...
    protected Component getLang(String ... elements) {
        if(Objects.isNull(elements) || elements.length==0) return new TextComponent(Constants.MODID);
        if(elements.length==1) return new TranslatableComponent(elements[0]+"."+Constants.MODID);
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, @NotNull List<Component> components, @NotNull TooltipFlag flag) {
        ...
        if(Objects.nonNull(trackID))
            components.add(new TextComponent(getLang("item","music_triggers_record","description")
                    .getString()+": "+trackID));
