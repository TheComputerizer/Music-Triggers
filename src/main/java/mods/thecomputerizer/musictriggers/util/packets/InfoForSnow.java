...
public class InfoForSnow {
    public static final Identifier id = new Identifier(MusicTriggersCommon.MODID, "infoforsnow");

    public static String decode(PacketByteBuf buf) {
        return ((String) buf.readCharSequence(buf.readableBytes(), StandardCharsets.UTF_8));
    }

    public static PacketByteBuf encode(String trigger, BlockPos p, UUID u) {
        String send = trigger+","+p.asLong()+","+u.toString();
