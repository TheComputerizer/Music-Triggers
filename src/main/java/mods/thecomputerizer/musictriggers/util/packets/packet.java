...
public class packet implements IMessageHandler<packet.packetMessage, IMessage> {
    ...
    public IMessage onMessage(packetMessage message, MessageContext ctx)
    {
        ...
        calculateFeatures.calculateStructAndSend(message.getDataTriggerName(), message.getDataStruct(), message.getDataBlockPos(), message.getDataInt(), message.getDataUUID());
        return null;
    }
