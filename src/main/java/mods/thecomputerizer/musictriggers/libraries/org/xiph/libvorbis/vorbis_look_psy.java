...
//	}
	...
	private void setup_tone_curves( float[] curveatt_dB, float binHz, int n, float center_boost, float center_decay_rate ) {
		...
		float[][][] workc = new float[ integer_constants.P_BANDS ][ integer_constants.P_LEVELS ][ integer_constants.EHMER_MAX ];
		float[][] athc = new float[ integer_constants.P_LEVELS ][ integer_constants.EHMER_MAX ];

		// float *brute_buffer=alloca(n*sizeof(*brute_buffer));
		float[] brute_buffer = new float[ n ];

		// float ***ret=_ogg_malloc(sizeof(*ret)*P_BANDS);
		tonecurves = new float[ integer_constants.P_BANDS ][ integer_constants.P_LEVELS ][ integer_constants.EHMER_MAX+2 ];
		...
		for (i=0; i< integer_constants.P_BANDS; i++ ) {
			...
			for (j=0; j< integer_constants.P_LEVELS; j++ ) {
				for (k=0; k< integer_constants.EHMER_MAX; k++ ) {
					float adj = center_boost+Math.abs(integer_constants.EHMER_OFFSET-k)*center_decay_rate;
			...
			for (j=0; j< integer_constants.P_LEVELS; j++ ) {
				...
				attenuate_curve( athc[j], 100.0f-j*10.f- integer_constants.P_LEVEL_0 );
			...
			for (j=1; j< integer_constants.P_LEVELS; j++ ) {
		...
		for (i=0; i< integer_constants.P_BANDS; i++ ) {
			...
			if (lo_curve<0)
				lo_curve=0;
			if (hi_curve>= integer_constants.P_BANDS)
				hi_curve= integer_constants.P_BANDS-1;

			for (m=0; m< integer_constants.P_LEVELS; m++ ) {
				...
				for ( k=lo_curve; k<=hi_curve; k++ ) {
					...
					for ( ; l<n; l++ ) {
						if (brute_buffer[l] > workc[k][m][integer_constants.EHMER_MAX-1] )
							brute_buffer[l]=workc[k][m][integer_constants.EHMER_MAX-1];
					}
				}

				// be equally paranoid about being valid up to next half ocatve

				if ( i+1 < integer_constants.P_BANDS ) {
				...
				// add fenceposts
				for (j=0; j< integer_constants.EHMER_OFFSET; j++ )
					if ( tonecurves[i][m][j+2] > -200.f )
						break;  
				tonecurves[i][m][0]=j;

				for (j= integer_constants.EHMER_MAX-1; j> integer_constants.EHMER_OFFSET+1; j-- )
					if ( tonecurves[i][m][j+2] > -200.f )
						break;
				tonecurves[i][m][1]=j;
			}
	...
	public void _vp_psy_init( vorbis_info_psy _vi, vorbis_info_psy_global gi, int _n, int _rate ) {
		...
		noiseoffset = new float[ integer_constants.P_NOISECURVES ][ n ];
		...
		for ( i=0; i<n; i++ ) {
			...
			if(halfoc>= integer_constants.P_BANDS-1)halfoc= integer_constants.P_BANDS-1;
...
// customized if else
				...
				if ( inthalfoc == integer_constants.P_BANDS-1 ) {

					if ( j == integer_constants.P_NOISECURVES-1 ) {
	...
	private void seed_loop( float[][][] curves, float[] f, float[] flr, float[] seed, float specmax ) {
		...
		for ( i=0; i < n; i++ ) {
			...
			if ( max+6.f > flr[i] ) {
				...
				if ( oc >= integer_constants.P_BANDS )
					oc = integer_constants.P_BANDS-1;
	...
	private void max_seeds( float[] seed, float[] flr ) {
		...
		while ( linpos+1 < n ) {
			...
			while ( pos+1 <= end ) {
				pos++;
				if ( (seed[pos] > integer_constants.NEGINF && seed[pos] < minV) || minV == integer_constants.NEGINF )
					minV = seed[pos];
			}
	...
	public void _vp_tonemask( float[] logfft, float[] logmask, float global_specmax, float local_specmax ) {
		...
		for ( i=0; i < total_octave_lines; i++ )
			seed[i] = integer_constants.NEGINF;
