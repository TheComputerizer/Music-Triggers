...
class envelope_lookup {
	...
	public envelope_lookup( int _ch, int _winlength, int _searchstep, float _minenergy, mdct_lookup _mdct, float[] _mdct_win,
		...
		band = new envelope_band[ integer_constants.VE_BANDS ];
	...
	public envelope_lookup( vorbis_info _vi ) {
		...
		for (int i = 0; i< integer_constants.VE_BANDS; i++)
		...
		for(int j = 0; j< integer_constants.VE_BANDS; j++) {
		...
		filter = new envelope_filter_state[ integer_constants.VE_BANDS*ch ];
		for (int i = 0; i < integer_constants.VE_BANDS*ch; i++ )
...
// local OOP variable rename stretch
		int stretch_local = Math.max( integer_constants.VE_MINSTRETCH, stretch/2 );
		float penalty = gi.stretch_penalty-(stretch/2- integer_constants.VE_MINSTRETCH);
		if ( penalty < 0.f )
			penalty = 0.f;
...
//		mdct.out( "", null );
		...
		decay *= (1./(integer_constants.VE_NEARDC+1));
		filters[offset2].nearptr++;
		if ( filters[offset2].nearptr >= integer_constants.VE_NEARDC )
			filters[offset2].nearptr = 0;
		decay = integer_constants.todB(decay)*.5f-15.f;
		...
		for ( i=0; i < n/2; i+=2 ) {
			...
			val = integer_constants.todB(val)*.5f;
			if ( val < decay )
				val=decay;
			if (val < minV)
				val=minV;
		...
		for (j=0; j < integer_constants.VE_BANDS; j++ ) {
...
// protected variable name this
			...
			if (filters[offset2+j].ampptr >= integer_constants.VE_AMP )
	...
	public void _ve_envelope_shift( int shift ) {
		...
		int smallsize = current/searchstep+ integer_constants.VE_POST; // adjust for placing marks ahead of ve->current
