...
public class vorbisenc {
	...
	public Object vorbis_encode_ctl( vorbis_info _vi, int number, Object arg ) {
		...
		switch (number) {
			...
			*/
			...
			case integer_constants.OV_ECTL_RATEMANAGE2_GET: {
			...
			case integer_constants.OV_ECTL_RATEMANAGE2_SET: {
			...
			case integer_constants.OV_ECTL_LOWPASS_GET: {
				
				return hi.lowpass_kHz;	
			}
			
			case integer_constants.OV_ECTL_LOWPASS_SET: {
			...
			case integer_constants.OV_ECTL_IBLOCK_GET: {
				
				return hi.impulse_noisetune;
			}

			case integer_constants.OV_ECTL_IBLOCK_SET: {
	...
	private void vorbis_encode_residue_setup( int number, int block, vorbis_residue_template res ) {
		...
		// lowpass setup/pointlimit
		{
			...
			if ( res.limit_type != 0 ) {
				if ( ci.hi.managed != 0 )
					freq=ci.psy_g_param.coupling_pkHz[integer_constants.PACKETBLOBS-1]*1000.0f;
				else
					freq=ci.psy_g_param.coupling_pkHz[integer_constants.PACKETBLOBS/2]*1000.0f;
				if (freq > nyq)
					freq=nyq;
			}
