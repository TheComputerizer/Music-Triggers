...
class oggpack_buffer {
	...
	public oggpack_buffer() {	// void oggpack_writeinit(oggpack_buffer *b)
		...
		ptr = 0;
		buffer = new byte[ integer_constants.BUFFER_INCREMENT ];
		buffer[0] = (byte)'\0';
		storage = integer_constants.BUFFER_INCREMENT;
	}
...
//        	 b->buffer=_ogg_realloc(b->buffer,b->storage+BUFFER_INCREMENT);
	...
	public boolean _vorbis_pack_info( vorbis_info vi ) {
		...
		oggpack_write( integer_constants.ilog2( ci.blocksizes[0] ), 4 );
		oggpack_write( integer_constants.ilog2( ci.blocksizes[1] ), 4 );
		oggpack_write( 1, 1 );
		    ...
		    for ( i=1; i < c.entries; i++ ) {
		    	...
		    	if ( current > last ) {
		    		for ( j=last; j < current; j++ ) {
		    			oggpack_write( i-count, integer_constants.ilog( c.entries-count ) );
		    			count=i;
		    		}
		    	}
		    }
	...
	public void floor1_pack( vorbis_info_floor1 info ) {
		...
		oggpack_write( integer_constants.ilog2( maxposit ), 4 );
		rangebits = integer_constants.ilog2( maxposit );
	...
	public int floor1_encode( vorbis_block vb, vorbis_look_floor1 look, int[] post, int[] ilogmask ) {
		...
		int[] out = new int[integer_constants.VIF_POSIT+2];
		...
		if ( post != null ) {
			...
			oggpack_write( out[0], integer_constants.ilog(look.quant_q-1) );
			oggpack_write( out[1], integer_constants.ilog(look.quant_q-1) );
			...
			for ( j=1; j < look.posts; j++ ) {
				...
				if ( hy == post[current] ) {
					...
					integer_constants.render_line0(lx,hx,ly,hy,ilogmask);
	...
	public void mapping0_pack( vorbis_info vi, vorbis_info_mapping0 info ) {
		...
		if ( info.coupling_steps > 0 ) {
			...
			for ( i=0; i < info.coupling_steps; i++ ) {
				oggpack_write( info.coupling_mag[i], integer_constants.ilog2( vi.channels ) );
				oggpack_write( info.coupling_ang[i], integer_constants.ilog2( vi.channels ) );
			}
		} else
			oggpack_write( 0, 1 );
