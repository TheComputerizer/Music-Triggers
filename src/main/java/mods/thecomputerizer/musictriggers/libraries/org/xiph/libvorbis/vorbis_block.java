...
public class vorbis_block {
	...
	public vorbis_block( vorbis_dsp_state v ) {
		...
		if ( v.analysisp > 0 ) {
			...
			for (int i = 0; i < integer_constants.PACKETBLOBS; i++ ) {
				if ( i == integer_constants.PACKETBLOBS/2 ) {
	...
	public int _vorbis_block_alloc( int bytes ) {
		...
		bytes = (bytes+(integer_constants.WORD_ALIGN-1)) & ~(integer_constants.WORD_ALIGN-1);
	...
	public boolean vorbis_analysis_blockout( vorbis_dsp_state v ) {
		...
		if ( v.W > 0 ){
			if ( (v.lW <= 0) || (v.nW <= 0) ) {
				vbi.blocktype = integer_constants.BLOCKTYPE_TRANSITION;
			} else {
				vbi.blocktype = integer_constants.BLOCKTYPE_LONG;
			}
		} else {
			if ( v._ve_envelope_mark() ) {
				vbi.blocktype = integer_constants.BLOCKTYPE_IMPULSE;
			} else {
				vbi.blocktype = integer_constants.BLOCKTYPE_PADDING;
			}
		}
...
//			pcm[i] = vbi.pcmdelay[i]+beginW;
	...
	public int[] floor1_fit( vorbis_look_floor1 look, int logmdct, float[] logmask) {
		  ...
		  lsfit_acc[] fits = new lsfit_acc[integer_constants.VIF_POSIT+1];
		  for (i=0; i < integer_constants.VIF_POSIT+1; i++ )
			  fits[i] = new lsfit_acc();
		  
		  int[] fit_valueA = new int[integer_constants.VIF_POSIT+2]; // index by range list position
		  int[] fit_valueB = new int[integer_constants.VIF_POSIT+2]; // index by range list position

		  int[] loneighbor = new int[integer_constants.VIF_POSIT+2]; // sorted index of range list position (+2)
		  int[] hineighbor = new int[integer_constants.VIF_POSIT+2];
		  int[] output = null;
		  int[] memo= new int[integer_constants.VIF_POSIT+2];
	...
	private float[][] _vp_quantize_couple_memo( vorbis_info_psy_global g, vorbis_look_psy p, vorbis_info_mapping0 vi, float[][] mdct) {
		...
		int limit = g.coupling_pointlimit[p.vi.blockflag][integer_constants.PACKETBLOBS/2];
...
// sets off the lsfit_acc data when flr[mdct+i]+info.twofitatten >= flr[i]
			...
			scale_dB = integer_constants.todB( scale ) + .345f;
			...
			logfft[0] = scale_dB + integer_constants.todB(pcm[i][0]) + .345f;
			local_ampmax[i] = logfft[0];
			for ( j=1; j<n-1; j+=2 ) {
				float temp = pcm[i][j]*pcm[i][j]+pcm[i][j+1]*pcm[i][j+1];
				temp = logfft[(j+1)>>1] = scale_dB + .5f * integer_constants.todB(temp) + .345f;
		...
		for ( i=0; i < vi.channels; i++ ) {
			...
			floor_posts[i] = new int[ integer_constants.PACKETBLOBS ][];
			
			for ( j=0; j < n/2; j++ )
				logfft[ logmdct+j ] = integer_constants.todB( mdct[j] ) + .345f;
...
// call to 
			...
			floor_posts[i][integer_constants.PACKETBLOBS/2] = floor1_fit( b.flr[info.floorsubmap[submap]], logmdct, logfft );
			
			// are we managing bitrate?  If so, perform two more fits for later rate tweaking (fits represent hi/lo)
			if ( vorbis_bitrate_managed() && (floor_posts[i][integer_constants.PACKETBLOBS/2] != null ) ) {
				...
				floor_posts[i][integer_constants.PACKETBLOBS-1] = floor1_fit( b.flr[info.floorsubmap[submap]], logmdct, logfft );
				...
				for (k=1; k < integer_constants.PACKETBLOBS/2; k++ ) {
					floor_posts[i][k] = floor1_interpolate_fit( b.flr[info.floorsubmap[submap]],
																floor_posts[i][0],
																floor_posts[i][integer_constants.PACKETBLOBS/2],
																k*65536/(integer_constants.PACKETBLOBS/2) );
				}
						
				for (k= integer_constants.PACKETBLOBS/2+1; k < integer_constants.PACKETBLOBS-1; k++ ) {
					floor_posts[i][k] = floor1_interpolate_fit( b.flr[info.floorsubmap[submap]],
																floor_posts[i][integer_constants.PACKETBLOBS/2],
																floor_posts[i][integer_constants.PACKETBLOBS-1],
																(k- integer_constants.PACKETBLOBS/2)*65536/(integer_constants.PACKETBLOBS/2) );
				}
			}
		}
		...
		for (k=(vorbis_bitrate_managed()?0: integer_constants.PACKETBLOBS/2); k <= (vorbis_bitrate_managed()? integer_constants.PACKETBLOBS-1: integer_constants.PACKETBLOBS/2); k++ ) {
	...
	public boolean vorbis_bitrate_addblock() {
		...
		if ( bm.avg_bitsper > 0 ) {
			...
			if ( bm.avg_reservoir+(this_bits-avg_target_bits) > desired_fill ) {
				...
				while ( choice+1< integer_constants.PACKETBLOBS && this_bits<avg_target_bits && bm.avg_reservoir+(this_bits-avg_target_bits)<desired_fill ) {
					choice++;
					this_bits = vbi.packetblob[choice].oggpack_bytes()*8;
		...
		if ( bm.min_bitsper > 0 ) {
			...
			if ( this_bits < min_target_bits ) {
				while ( bm.minmax_reservoir-(min_target_bits-this_bits) < 0 ) {
					choice++;
					if ( choice >= integer_constants.PACKETBLOBS )
						break;
					this_bits = vbi.packetblob[choice].oggpack_bytes()*8;
				}
			}
		...
		if ( choice < 0 ) {
			...
			if ( choice >= integer_constants.PACKETBLOBS )
				choice = integer_constants.PACKETBLOBS-1;
