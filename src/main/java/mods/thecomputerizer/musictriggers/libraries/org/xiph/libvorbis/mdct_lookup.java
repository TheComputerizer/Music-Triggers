...
class mdct_lookup {
	...
	private void mdct_butterfly_16( float[] x, int off ) {
		...
		x[off+8]  += x[off+0];
		x[off+9]  += x[off+1];
		x[off+0]   = ((r0   + r1) * integer_constants.cPI2_8);
		x[off+1]   = ((r0   - r1) * integer_constants.cPI2_8);
		...
		x[off+12] += x[off+4];
		x[off+13] += x[off+5];
		x[off+4]   = ((r0   - r1) * integer_constants.cPI2_8);
		x[off+5]   = ((r0   + r1) * integer_constants.cPI2_8);
	...
	private void mdct_butterfly_32( float[] x, int off ) {
		...
		x[off+28] +=         x[off+12];           
		x[off+29] +=         x[off+13];
		x[off+12]  = ( r0 * integer_constants.cPI1_8  -  r1 * integer_constants.cPI3_8 );
		x[off+13]  = ( r0 * integer_constants.cPI3_8  +  r1 * integer_constants.cPI1_8 );

		r0     = x[off+26] - x[off+10];
		r1     = x[off+27] - x[off+11];
		x[off+26] +=         x[off+10];
		x[off+27] +=         x[off+11];
		x[off+10]  = (( r0  - r1 ) * integer_constants.cPI2_8);
		x[off+11]  = (( r0  + r1 ) * integer_constants.cPI2_8);

		r0     = x[off+24] - x[off+8];
		r1     = x[off+25] - x[off+9];
		x[off+24] += x[off+8];
		x[off+25] += x[off+9];
		x[off+8]   = ( r0 * integer_constants.cPI3_8  -  r1 * integer_constants.cPI1_8 );
		x[off+9]   = ( r1 * integer_constants.cPI3_8  +  r0 * integer_constants.cPI1_8 );
		...
		x[off+20] += x[off+4];
		x[off+21] += x[off+5];
		x[off+4]   = ( r1 * integer_constants.cPI1_8  +  r0 * integer_constants.cPI3_8 );
		x[off+5]   = ( r1 * integer_constants.cPI3_8  -  r0 * integer_constants.cPI1_8 );

		r0     = x[off+2]  - x[off+18];
		r1     = x[off+3]  - x[off+19];
		x[off+18] += x[off+2];
		x[off+19] += x[off+3];
		x[off+2]   = (( r1  + r0 ) * integer_constants.cPI2_8);
		x[off+3]   = (( r1  - r0 ) * integer_constants.cPI2_8);

		r0     = x[off+0]  - x[off+16];
		r1     = x[off+1]  - x[off+17];
		x[off+16] += x[off+0];
		x[off+17] += x[off+1];
		x[off+0]   = ( r1 * integer_constants.cPI3_8  +  r0 * integer_constants.cPI1_8 );
		x[off+1]   = ( r1 * integer_constants.cPI1_8  -  r0 * integer_constants.cPI3_8 );
