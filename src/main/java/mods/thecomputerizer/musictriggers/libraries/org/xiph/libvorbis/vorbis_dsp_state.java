...
// PTR CITY
	...
	private boolean _vds_shared_init( boolean encp ) {
		...
		backend_state.modebits = integer_constants.ilog2( ci.modes );

		backend_state.transform[0][0].mdct_init( ci.blocksizes[0]>>>hs );
		backend_state.transform[1][0].mdct_init( ci.blocksizes[1]>>>hs );

		// Vorbis I uses only window type 0
		backend_state.window[0] = integer_constants.ilog2(ci.blocksizes[0])-6;
		backend_state.window[1] = integer_constants.ilog2(ci.blocksizes[1])-6;
...
// by pcm_current to alter the .wav buffer
	...
	public boolean vorbis_bitrate_flushpacket( ogg_packet op ) {
		...
		int choice = integer_constants.PACKETBLOBS/2;
	...
	public int _ve_envelope_search() {
		...
		int last = pcm_current/ve.searchstep- integer_constants.VE_WIN;
		if ( first < 0 )
			first=0;
		
		// make sure we have enough storage to match the PCM
		if ( last+ integer_constants.VE_WIN+ integer_constants.VE_POST > ve.storage ) {
			ve.storage=last+ integer_constants.VE_WIN+ integer_constants.VE_POST; // be sure
		...
		for ( j=first; j < last; j++ ) {
			...
			ve.stretch++;
			if ( ve.stretch > integer_constants.VE_MAXSTRETCH*2 )
				ve.stretch = integer_constants.VE_MAXSTRETCH*2;
			
			for ( i=0; i < ve.ch; i++ ) {
				// float *pcm = pcm[i] + ve.searchstep*(j);
				// ret |= _ve_amp( ve, gi, _pcm, ve.band, ve.filter+i*VE_BANDS, j );
				ret |= ve._ve_amp( gi, pcm[i], ve.searchstep*(j), ve.band, ve.filter, i* integer_constants.VE_BANDS, j );
			}
			
			ve.mark[j+ integer_constants.VE_POST] = 0;
			if ( (ret&1) > 0 ) {
				ve.mark[j] = 1;
				ve.mark[j+1] = 1;
			}
