...
public class MusicTriggersRecord extends Item {
        ...
        if (state.getBlock() instanceof MusicRecorder mr) {
            ...
            if (!ctx.getLevel().isClientSide && ctx.getPlayer()!=null) {
                ...
                if(stack.getOrCreateTag().contains("trackID") && stack.getOrCreateTag().contains("channelFrom")) {
                    ...
                    PacketHandler.sendTo(new PacketJukeBoxCustom(ctx.getClickedPos(),stack.getOrCreateTag().getString("channelFrom"),
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> tooltip, TooltipFlag context) {
        if(stack.getOrCreateTag().contains("trackID"))
            tooltip.add(MutableComponent.create(new LiteralContents(getDescription().getString()+": "+
                    stack.getOrCreateTag().getString("trackID"))));
        else tooltip.add(MutableComponent.create(
                new TranslatableContents("item.musictriggers.music_triggers_record.blank_description")));
    }

    public MutableComponent getDescription() {
        return MutableComponent.create(new TranslatableContents(this.getDescriptionId() + ".desc"));
    }
