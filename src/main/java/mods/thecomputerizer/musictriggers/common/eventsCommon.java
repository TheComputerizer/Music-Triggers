        ...
        for (Map.Entry<BlockPos, ItemStack> blockPosItemStackEntry : recordHolder.entrySet()) {
            ...
            if(recordHolder.get(blockPos)!=null && !recordHolder.get(blockPos).isEmpty() && (recordHolder.get(blockPos).getItem() instanceof BlankRecord || (recordHolder.get(blockPos).getItem() instanceof MusicTriggersRecord && recordWorld.get(blockPos).getBlockState(blockPos).get(MusicRecorder.HAS_DISC)))) {
                ...
                if(randomNum+tickCounter.get(blockPos)>=6000) {
                    LightningEntity lightning = EntityType.LIGHTNING_BOLT.create(recordWorld.get(blockPos));
                    if(lightning!=null) {
                    ...
                    if (recordHolder.get(blockPos).getItem() instanceof BlankRecord) {
                        ItemStack stack = MusicTriggersItems.MUSIC_TRIGGERS_RECORD.getDefaultStack();
                        stack.getOrCreateNbt().putString("channelFrom",currentChannel);
                        stack.getOrCreateNbt().putString("trackID",currentChannelSongs.get(recordUUID.get(blockPos)).get(currentChannel));
                        stack.getOrCreateNbt().putString("triggerID",activeTriggerList.get(recordUUID.get(blockPos)).get(currentChannel).get(random.nextInt(activeTriggerList.get(recordUUID.get(blockPos)).get(currentChannel).size())));
                        ...
                        ItemStack stack = MusicTriggersItems.MUSIC_TRIGGERS_RECORD.getDefaultStack();
                        stack.getOrCreateNbt().putString("channelFrom",currentChannel);
                        stack.getOrCreateNbt().putString("trackID",randomMenuSong);
                        stack.getOrCreateNbt().putString("triggerID","menu");
                        recordHolder.put(blockPos, stack);
                    }
                }
            }
