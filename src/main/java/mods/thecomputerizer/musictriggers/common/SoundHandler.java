    ...
    public static void registerSounds() {
        ...
        for (Map.Entry<String, Map<String, String[]>> stringListEntry : configToml.triggerholder.entrySet()) {
            ...
            if(triggers.size()==1) {
                ...
                if(!TriggerSongMap.containsKey(trigger)) {
                    TriggerSongMap.put(trigger, new HashMap<>());
                }
                if(!TriggerSongMap.get(trigger).containsKey(songEntry)) {
                    TriggerSongMap.get(trigger).put(songEntry,configToml.triggerholder.get(songEntry).get(trigger)[10]);
                }
                if(!TriggerInfoMap.containsKey(trigger)) {
                    TriggerInfoMap.put(trigger, configToml.triggerholder.get(songEntry).get(trigger));
                }
