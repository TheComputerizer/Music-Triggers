...
public class ExecutorTools {
  ...
  public static void shutdownExecutor(ExecutorService executorService, String description) {
    ...
    MusicTriggers.logger.debug("Shutting down executor {}", description);

    executorService.shutdownNow();

    try {
      if (!executorService.awaitTermination(WAIT_TIME, TimeUnit.MILLISECONDS)) {
        MusicTriggers.logger.debug("Executor {} did not shut down in {}", description, WAIT_TIME);
      } else {
        MusicTriggers.logger.debug("Executor {} successfully shut down", description);
      }
    } catch (InterruptedException e) {
      MusicTriggers.logger.debug("Received an interruption while shutting down executor {}", description);
