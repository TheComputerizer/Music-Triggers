...
public class DefaultAudioPlayerManager implements AudioPlayerManager {
  ...
  private Future<Void> handleLoadRejected(String identifier, AudioLoadResultHandler resultHandler, RejectedExecutionException e) {
    ...
    ExceptionTools.log(MusicTriggers.logger, exception, "queueing item " + identifier);
    ...
    return () -> {
      ...
      try {
        if (!checkSourcesForItem(reference, resultHandler, reported)) {
          MusicTriggers.logger.debug("No matches for track with identifier {}.", reference.identifier);
          resultHandler.noMatches();
        }
      } catch (Throwable throwable) {
        if (reported[0]) {
          MusicTriggers.logger.warn("Load result handler for {} threw an exception", reference.identifier, throwable);
        } else {
  ...
  private void dispatchItemLoadFailure(String identifier, AudioLoadResultHandler resultHandler, Throwable throwable) {
    ...
    ExceptionTools.log(MusicTriggers.logger, exception, "loading item " + identifier);
    ...
    for (AudioSourceManager sourceManager : sourceManagers) {
      ...
      if (item != null) {
        if (item instanceof AudioTrack) {
          MusicTriggers.logger.debug("Loaded a track with identifier {} using {}.", reference.identifier, sourceManager.getClass().getSimpleName());
          reported[0] = true;
          resultHandler.trackLoaded((AudioTrack) item);
        } else if (item instanceof AudioPlaylist) {
          MusicTriggers.logger.debug("Loaded a playlist with identifier {} using {}.", reference.identifier, sourceManager.getClass().getSimpleName());
          reported[0] = true;
