...
public class GarbageCollectionMonitor implements NotificationListener, Runnable {
  ...
  public void enable() {
    if (enabled.compareAndSet(false, true)) {
      ...
      MusicTriggers.logger.info("GC monitoring enabled, reporting results every 2 minutes.");
    }
  }
  ...
  public void disable() {
    if (enabled.compareAndSet(true, false)) {
      ...
      MusicTriggers.logger.info("GC monitoring disabled.");
    }
  }
    ...
    for (GarbageCollectorMXBean gcBean : ManagementFactory.getGarbageCollectorMXBeans()) {
      if (gcBean instanceof NotificationEmitter) {
        try {
          ...
          MusicTriggers.logger.debug("No listener found on bean {}, should have been there.", gcBean, e);
        }
      }
    }
  }
  ...
  public void run() {
    ...
    if (hasBadLatency) {
      MusicTriggers.logger.warn("Suspicious GC results for the last 2 minutes: {}", statistics);
    } else {
      MusicTriggers.logger.debug("GC results for the last 2 minutes: {}", statistics);
    }
  }
}
