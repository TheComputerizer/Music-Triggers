...
public class ExceptionTools {
  ...
  public static void log(Logger log, FriendlyException exception, Object context) {
    switch (exception.severity) {
      case COMMON:
        MusicTriggers.logger.debug("Common failure for {}: {}", context, exception.getMessage());
        break;
      case SUSPICIOUS:
        MusicTriggers.logger.warn("Suspicious exception for {}", context, exception);
        break;
      case FAULT:
      default:
        MusicTriggers.logger.error("Error in {}", context, exception);
        break;
    }
  }
  ...
  public static void closeWithWarnings(AutoCloseable closeable) {
    try {
      ...
      MusicTriggers.logger.warn("Failed to close.", e);
    }
  }
  ...
  public static class DefaultErrorDebugInfoHandler implements ErrorDebugInfoHandler {
    ...
    public void handle(ErrorDebugInfo debugInfo) {
      MusicTriggers.logger.warn("{} EID: {}, {}: {}", debugInfo.message, debugInfo.errorId, debugInfo.name, debugInfo.value);
    }
  }
}
