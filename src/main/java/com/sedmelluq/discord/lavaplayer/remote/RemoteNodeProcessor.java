...
public class RemoteNodeProcessor implements RemoteNode, Runnable {
  ...
  public void startPlaying(RemoteAudioTrackExecutor executor) {
    ...
    if (playingTracks.putIfAbsent(executor.getExecutorId(), executor) == null) {
      ...
      MusicTriggers.logger.info("Sending request to play {} {} from position {}", track.getIdentifier(), executor.getExecutorId(), position);
  ...
  public void trackEnded(RemoteAudioTrackExecutor executor, boolean notifyNode) {
    if (playingTracks.remove(executor.getExecutorId()) != null) {
      MusicTriggers.logger.info("Track {} removed from node {} (context {})", executor.getTrack().getIdentifier(), nodeAddress, executor.getExecutorId());

      if (notifyNode) {
        MusicTriggers.logger.info("Notifying node {} of track stop for {} (context {})", nodeAddress, executor.getTrack().getIdentifier(), executor.getExecutorId());
  ...
  public void run() {
    if (closed || !threadRunning.compareAndSet(false, true)) {
      MusicTriggers.logger.debug("Not running node processor for {}, thread already active.", nodeAddress);
      return;
    }

    MusicTriggers.logger.debug("Trying to connect to node {}.", nodeAddress);
    ...
    try (HttpInterface httpInterface = httpInterfaceManager.getInterface()) {
      ...
      MusicTriggers.logger.info("Node {} processing was stopped.", nodeAddress);
      Thread.currentThread().interrupt();
    } catch (IOException e) {
      if (aliveTickCounter > 0) {
        MusicTriggers.logger.error("Node {} went offline with exception.", nodeAddress, e);
      } else {
        MusicTriggers.logger.debug("Retry, node {} is still offline.", nodeAddress);
      }
    } catch (Throwable e) {
      MusicTriggers.logger.error("Node {} appears offline due to unexpected exception.", nodeAddress, e);
      ...
      if (!closed) {
        ...
        if (aliveTickCounter == -1) {
          MusicTriggers.logger.info("Node {} loop ended, retry scheduled in {}.", nodeAddress, delay);
        }

        scheduledExecutor.schedule(this, delay, TimeUnit.MILLISECONDS);
      } else {
        MusicTriggers.logger.info("Node {} loop ended, node was removed.", nodeAddress);
      }
    }
  }
  ...
  private boolean dispatchOneTick(HttpInterface httpInterface, TickBuilder tickBuilder) throws Exception {
    ...
    try {
      ...
      if (connectionState.compareAndSet(ConnectionState.PENDING.id(), ConnectionState.ONLINE.id())) {
        MusicTriggers.logger.info("Node {} came online.", nodeAddress);
      } else if (connectionState.get() != ConnectionState.ONLINE.id()) {
        MusicTriggers.logger.warn("Node {} received successful response, but had already lost control of its tracks.", nodeAddress);
        return false;
      }
  ...
  private byte[] buildRequestBody() throws IOException {
    ...
    if (queuedCount > 0) {
      MusicTriggers.logger.debug("Including {} queued messages in the request to {}.", queuedCount, nodeAddress);
    }
  ...
  private boolean handleResponseBody(InputStream inputStream, TickBuilder tickBuilder) {
    ...
    try {
      ...
      MusicTriggers.logger.error("Node {} processing thread was interrupted.", nodeAddress);
      Thread.currentThread().interrupt();
      return false;
    } catch (Throwable e) {
      MusicTriggers.logger.error("Error when processing response from node {}.", nodeAddress, e);
      ExceptionTools.rethrowErrors(e);
    } finally {
  ...
  private void handleTrackStartResponse(TrackStartResponseMessage message) {
    if (message.success) {
      MusicTriggers.logger.debug("Successful start confirmation from node {} for executor {}.", nodeAddress, message.executorId);
    } else {
      ...
      if (executor != null) {
        ...
        MusicTriggers.logger.debug("Received failed track start for an already stopped executor {} from node {}.", message.executorId, nodeAddress);
      }
    }
  }
  ...
  private void handleNodeStatistics(NodeStatisticsMessage message) {
    MusicTriggers.logger.trace("Received stats from node: {} {} {} {}", message.playingTrackCount, message.totalTrackCount,
  ...
  public synchronized void processHealthCheck(boolean terminate) {
    ...
    if (!terminate) {
      MusicTriggers.logger.warn("Bringing node {} offline since last response from it was {}ms ago.", nodeAddress, System.currentTimeMillis() - lastAliveTime);
    }
