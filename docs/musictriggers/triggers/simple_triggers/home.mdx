...
Playable when the respawn position of the player for the current dimension is within the specified range.

Accepts `active_cooldown`, `detection_range`, `detection_y_ratio`, `fade_in`, `fade_out`, `max_tracks`, `not`,
...
detection_range <default: 16>
    ...
    check for a respawn position in.
