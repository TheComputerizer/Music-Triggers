Playable when the specified statistic is higher than the specified threshold for the player.

Accepts `active_cooldown`, `display_matcher`, `display_name`, `fade_in`, `fade_out`, `level`, `max_tracks`, `not`,
...
display_name <default: [ any ]>
    The display name of the stat must match one of the entries here
...
level <default: 0>
    Any matching stat must be greater than or equal to this value for the
    trigger to activate.
...
resource_name <default: [ any ]>
    The resource name or ID of the stat must match one of the entries here
