...
Playable when the Y position of the player is within the specified bound.

Accepts `active_cooldown`, `check_above_level`, `check_for_sky`, `fade_in`, `fade_out`, `level`, `max_tracks`, `not`,
...
Requires Parameters `identifier` AND `level`
...
check_above_level <default: false>
    Reverses the level check so that the y level of the player must be less
    than the set value instead of greater than.

check_for_sky <default: true>
    If check_above_level is not enabled, this makes it so there must be not
    be sky visible directly above the player.
...
level <default: 0>
    The y level of the player must be greater than this value for the trigger
    to be active.
