Playable when there is a tornado within the specified range that is at or above the specified level. Requires
[Weather, Storms & Tornadoes](https://www.curseforge.com/minecraft/mc-mods/weather-storms-tornadoes) (1.12.2)

Accepts `active_cooldown`, `detection_range`, `fade_in`, `fade_out`, `level`, `max_tracks`, `not`,
...
detection_range <default: 16>
    Determines the range around the player that this trigger will check for a
    tornado in.
...
level <default: 0>
    The minimum strength level (up to 5) that a tornado in range must be in
    order for this trigger to activate.
