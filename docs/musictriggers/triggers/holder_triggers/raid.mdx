...
Playable when there is active raid at or above the specified wave near the player. Requires 1.16.5+

Accepts `active_cooldown`, `fade_in`, `fade_out`, `max_tracks`, `not`, `passive_persistence`, `persistence`,
 `play_when_paused`, `priority`, `raid_status`, `start_as_disabled`, `ticks_before_active`, `ticks_before_audio`,
 `ticks_between_audio`, `toggle_inactive_playable`, `toggle_save_status`, and `wave`

Requires Parameter `identifier`
...
raid_status <default: [ "any" ]>
    The status of the current raid must be one of the values in this parameter.
    A value of "any" means this parameter will be ignored.
    Accepted values are "LOSS", "ONGOING", "STOPPED", or "VICTORY"
...
wave <default: 0>
    Determines the (inclusive) minimum wave number of the current raid needed
    for this trigger to activate.
```