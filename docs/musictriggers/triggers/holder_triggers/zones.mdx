...
Playable while the position of the player is above the specified min values and below the specified max values.
...
Requires Parameters `identifier` AND `zone_min_x` AND `zone_max_x` AND `zone_min_y` AND `zone_max_y` AND `zone_min_z`
 AND `zone_max_z`
