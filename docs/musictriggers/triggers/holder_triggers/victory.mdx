...
**Requires Music Triggers to be on the server**

Playable when the targets of a mob or pvp trigger which has a victory_id parameter specified are killed. It is
recommended to use the persistence parameter with this trigger.

Accepts `active_cooldown`, `fade_in`, `fade_out`, `max_tracks`, `not`, `passive_persistence`, `persistence`,
 `play_when_paused`, `priority`, `start_as_disabled`, `ticks_before_active`, `ticks_before_audio`,
 `ticks_between_audio`, `toggle_inactive_playable`, `toggle_save_status`, and `victory_timeout`

Requires Parameters `identifier` AND `persistence`
...
victory_timeout <default: 20>
    The time in ticks it will take this trigger to time out after a
    connecting trigger is no longer active. For example, with a
    victory_timeout of 20 and a connecting mob trigger, the mob trigger will
    be able to activate the victory trigger for up to 20 ticks after it is
    no longer active. Only killing entities that previously activated the
    connecting trigger will count.
```