...
Playable for 1 tick after an advancement has been granted. It is recommended to use the persistence parameter with this
trigger.
...
Requires Parameters `identifier` AND `persistence` AND ( `display_name` OR `resource_name` )
...
display_name <default: [ any ]>
    The display name of the advancement must match one of the entries here
...
resource_name <default: [ any ]>
    The resource name or ID of the advancement must match one of the entries
