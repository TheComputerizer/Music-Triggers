Playable when the light level matches the specified parameters.

Accepts `active_cooldown`, `fade_in`, `fade_out`, `level`, `light_type`, `max_tracks`, `not`, `passive_persistence`,
 `persistence`, `play_when_paused`, `priority`, `start_as_disabled`, `ticks_before_active`, `ticks_before_audio`,
...
level <default: 7>
    The level of the specified light_type must be equal to or less than this
    value for the trigger to activate.

light_type <default: any>
    Determines the type of light the level parameter will check for. Accepts
    block, sky, or any (either block or sky).
