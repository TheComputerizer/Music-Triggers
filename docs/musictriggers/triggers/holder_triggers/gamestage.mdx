...
Playable when the player either has or does not have the specified gamestage depending on the specified whitelist.
Requires [Game Stages](https://www.curseforge.com/minecraft/mc-mods/game-stages) (1.12.2, 1.16.5, [1.18.2, 1.19.2] Forge)

Accepts `active_cooldown`, `display_matcher`, `display_name`, `fade_in`, `fade_out`, `is_whitelist`, `max_tracks`,
 `not`, `passive_persistence`, `persistence`, `play_when_paused`, `priority`, `resource_matcher`, `resource_name`,
...
display_name <default: [ any ]>
    The display name of the stage must match one of the entries here
...
is_whitelist <default: true>
    Determines whether the player is required to have or be missing the
    matching stage(s) for this trigger to be active.
...
resource_name <default: [ any ]>
    The resource name or ID of the stage must match one of the entries here
