...
Accepts `active_cooldown`, `champion`, `detection_range`, `detection_y_ratio`, `display_matcher`, `display_name`,
 `fade_in`, `fade_out`, `horde_health_percentage`, `horde_targeting_percentage`, `infernal`, `max_health`,
 `max_entities`, `max_tracks`, `min_entities`, `min_health`, `mob_nbt`, `mob_targeting`, `not`, `passive_persistence`,
 `persistence`, `play_when_paused`, `priority`, `resource_matcher`, `resource_name`, `start_as_disabled`,
 `ticks_before_active`, `ticks_before_audio`, `ticks_between_audio`, `toggle_inactive_playable`, `toggle_save_status`,
 `victory_id`, and `victory_percentage`
...
min_health <default: 0>
    Determines the (inclusive) miniumum health percentage an entity can have
        in order for the trigger to activate.
